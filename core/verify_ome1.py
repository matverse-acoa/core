...
def extract_pose(receipt: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    """
    pose = receipt["pose"] if "pose" in receipt and isinstance(receipt["pose"], dict) else receipt
...
def main() -> None:
    ...
    if not os.path.exists(hashes_path):
        die("hashes.json not found (run: make auditpack OR place hashes.json at repo root)")
    ...
    if computed_residue != expected_residue:
        die(f"residue_hash mismatch: expected {expected_residue}, got {computed_residue}")
