...
class Ledger:
    ...
    def append(self, kind: str, payload: Dict[str, Any]) -> Tuple[int, str]:
        ...
        with self._lock:
            cur = self.db.execute(
                (
                    "INSERT INTO events(ts,kind,payload_json,prev_hash,this_hash) "
                    "VALUES(?,?,?,?,?)"
                ),
    ...
    def put_evidence_append_only(self, evidence_id: str, pose: Dict[str, Any]) -> str:
        ...
        with self._lock:
            cur = self.db.execute(
                "SELECT pose_hash FROM evidence WHERE evidence_id=?",
                (evidence_id,),
            )
            row = cur.fetchone()
            if row:
            ...
            self.db.execute(
                (
                    "INSERT INTO evidence(evidence_id,ts,pose_json,pose_hash) "
                    "VALUES(?,?,?,?)"
                ),
        ...
        self.append(
            "EVIDENCE_STORED",
            {"evidence_id": evidence_id, "pose_hash": pose_hash},
        )
        return pose_hash

    def get_evidence(self, evidence_id: str) -> Optional[Dict[str, Any]]:
        cur = self.db.execute(
            "SELECT pose_json FROM evidence WHERE evidence_id=?",
            (evidence_id,),
        )
        row = cur.fetchone()
